(function() {
    ...
    function( Backbone, Communicator, timeslider, globals) {
        var TimeSliderView = Backbone.Marionette.ItemView.extend({
            ...
            fetchWCS: function(start, end, params, callback){
                var request = this.url + '?service=WCS&version=2.0.1&request=DescribeCoverage&coverageid='+this.id;

                d3.xml(request, function(error, data){
                            ...
                            switch (product.get('timeSliderProtocol')){
                                ...
                                case 'WCS':
                                  ...
                                  this.slider.addDataset({
                                    ...
                                    color: product.get('color'),
                                    records: null,
                                    source: {fetch: this.fetchWCS.bind(attrs)}
                                  });
                                  break;
                                case 'WPS':
                                  attrs = {
